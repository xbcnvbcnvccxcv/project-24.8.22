...
# LOWESS 회귀선을 사용하여 산점도와 회귀선 그리기
plt.figure(figsize=(10, 6))
sns.regplot(x=X2, y=Y2, scatter_kws={'color':'black', 's':30, 'alpha':0.5}, line_kws={'color':'black'}, lowess=True)

# 그래프에 제목 추가
plt.title("Total Precipitation vs Number of Heatstroke Patients with LOWESS Regression Line")
plt.xlabel("Total Precipitation (mm)")
plt.ylabel("Number of Heatstroke Patients")
plt.grid(True)

# 결과를 파일로 저장
plt.savefig(os.path.join(output_dir, 'Total_Precipitation_LOWESS_Custom.png'))
plt.show()
